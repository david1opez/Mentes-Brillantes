...
router.post('/321fc4EE-ec01-4562-8f39-93106fAef42e', (req, res) => {
    ...
    //IF THE USER IS A PARTICIPANT FROM THE CONTEST
    if(action == "PARTICIPANT") {
        //CHECK IF CODE MATCHES WITH A TEAM
        var team = null;

        for(var i = 0; i < CODES.length; i++) {
          if(code == CODES[i]){team = TEAM_NAMES[i]}
        }

        if(team != null) {
          db.ref(`${USERS}/` + PARTICIPANTS).child(uid).set(
            {
              nombre: username,
              correo: email,
              votosRestantes: cryptr.encrypt(3),
        ...
        //UPLOAD USERS DATA TO THE FOREIGNS DATABASE
        //THIS DOES THE SAME THING AS ABOVE, BUT DOES NOT INCLUDE THE SECTION AND
    }
});
